	...
	if(H){
		...
		switch(FUNCTION_ID){
			...
			case BBDBM_CD: /* Bernoulli_BernoulliDBM4Reconstruction trained by Contrastive Divergence */
				...
				gsl_matrix_free(Param);
			break;
			case BBDBM_PCD: /* Bernoulli_BernoulliDBM4Reconstruction trained by Persistent Contrastive Divergence */
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
				n_gibbs_sampling = va_arg(arg, int);
				...
				gsl_matrix_free(Param);
			break;
			case BBDBM_FPCD: /* Bernoulli_BernoulliDBM4Reconstruction trained by Fast Persistent Contrastive Divergence */
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
				n_gibbs_sampling = va_arg(arg, int);
				...
				gsl_matrix_free(Param);
			break;
			case 20: /* Bernoulli_BernoulliRBM4Reconstruction with Dropout */
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
						
				for(i = 0; i < H->m; i++){
				    f = Evaluate(g, gsl_matrix_get(H->HM, i, 0), gsl_matrix_get(H->HM, i, 1), gsl_matrix_get(H->HM, i, 2), gsl_matrix_get(H->HM, i, 3), gsl_matrix_get(H->HM, i, 4), gsl_matrix_get(H->HM, i, 5), n_epochs, batch_size, gsl_vector_get(H->LB, 1), gsl_vector_get(H->UB, 1)); 
				    gsl_vector_set(H->fitness, i, f);
				    if(f < H->best_fitness){
					H->best = i;
					H->best_fitness = f;
	...
	if((H) && (h)){
		...
		switch(FUNCTION_ID){
			...
			case BBDBM_FPCD: /* Bernoulli_BernoulliDBM4Reconstruction trained by Fast Persistent Contrastive Divergence */
				...
				gsl_matrix_free(Param);
			break;
			case 20: /* Bernoulli-Bernoulli RBM trained by Contrastive Divergence with Dropout */
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
...
void runHS(HarmonyMemory *H, prtFun EvaluateFun, int FUNCTION_ID, ...){
    ...
    if(H){
		...
		for(t = 1; t <= H->max_iterations; t++){
			...
			fprintf(stderr, "\nOK (minimum fitness value %lf)", H->best_fitness);
			fprintf(stdout,"%d %lf\n", t, H->best_fitness);
...
void runSGHS(HarmonyMemory *H, prtFun EvaluateFun, int FUNCTION_ID, ...){
	...
	fprintf(stderr," OK.");
        ...
        /* Initializing HMCR~N(HMCRm,0.01) and PAR~N(PARm,0.05) according to Section 3.2.1 */
...
QHarmonyMemory *CreateQHarmonyMemory(int m, int n){
	...
	if((m < 1) || (n < 1)){
		fprintf(stderr,"\nInvalid parameters @CreateQHarmonyMemory.\n");
		return NULL;
	}
...
QHarmonyMemory *ReadQHarmoniesFromFile(char *fileName){
	...
	double LB, UB;
        ...
        fp = fopen(fileName, "r");
        if(!fp){
                fprintf(stderr,"\nunable to open file %s @ReadQHarmonyMemoryFromFile.\n", fileName);
                return NULL;
        }
...
/* It creates a new quaternion-based harmony for SGHS
Parameters: [H]
H: harmony memory */
gsl_matrix *CreateNewQHarmony4SGHS(QHarmonyMemory *H){
	if(H){
		int i, j, index;
		gsl_matrix *h = NULL;
		const gsl_rng_type *T = NULL;
		...
		for(i = 0; i < H->n; i++){
			...
			if(H->HMCR >= p){
				...
				if(p >= 0.5){
					for (j = 0; j < 4; j++)
						gsl_matrix_set(h, j, i, gsl_matrix_get(H->HM[index], j, i)+(p2*H->bw));
				}
				else{
					for (j = 0; j < 4; j++)
						gsl_matrix_set(h, j, i, gsl_matrix_get(H->HM[index], j, i)-(p2*H->bw));
		...
		fprintf(stderr,"\nThere is no harmony memory allocated @CreateNewQHarmony4SGHS.\n");
		return NULL;
	}
}
	...
	if((H) && (h)){
		...
		switch(FUNCTION_ID){
			...
			case BBDBN_FPCD: /* Bernoulli_BernoulliDBN4Reconstruction using Fast Persistent Contrastive Divergence*/
				...
				gsl_matrix_free(Param);
			break;
			case BBDBM_CD: /* Bernoulli_BernoulliDBM4Reconstruction using Contrastive Divergence*/
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
				n_gibbs_sampling = va_arg(arg, int);
			...
			break;
			case BBDBM_PCD: /* Bernoulli_BernoulliDBM4Reconstruction using Persistent Contrastive Divergence*/
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
				n_gibbs_sampling = va_arg(arg, int);
				...
				gsl_matrix_free(Param);
			break;
			case BBDBM_FPCD: /* Bernoulli_BernoulliDBM4Reconstruction using Fast Persistent Contrastive Divergence*/
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
				n_gibbs_sampling = va_arg(arg, int);
...
void EvaluateQHarmonies(QHarmonyMemory *H, prtFun Evaluate, int FUNCTION_ID, va_list arg){
	if(H){
		...
		switch(FUNCTION_ID){
			...
			case BBDBM_CD: /* Bernoulli_BernoulliDBM4Reconstruction using Persistent Contrastive Divergence */
				g = va_arg(arg, Subgraph *);
				n_epochs = va_arg(arg, int);
				batch_size = va_arg(arg, int);
				n_gibbs_sampling = va_arg(arg, int);
...
/* It executes the quaternion-based Harmony Search for function minimization ---
Parameters: [H, EvaluateFun, FUNCTION_ID, ... ]
H: search space
EvaluateFun: pointer to the function used to evaluate bats
FUNCTION_ID: id of the function registered at opt.h
... other parameters of the desired function */
void runQHS(QHarmonyMemory *H, prtFun EvaluateFun, int FUNCTION_ID, ...){
    va_list arg, argtmp;
    ...
    if(H){
		...
		fprintf(stderr,"\nInitial evaluation of the Q-harmony memory ...");
    ...
    }else fprintf(stderr,"\nThere is no search space allocated @runQHS.\n");
    va_end(arg);
}
...
... other parameters of the desired function
-> This implementation is based on the paper "A self-adaptive global best harmony search algorithm for continuous optimization problems". */
void runQSGHS(QHarmonyMemory *H, prtFun EvaluateFun, int FUNCTION_ID, ...){
    va_list arg, argtmp;
    ...
    }else fprintf(stderr,"\nThere is no search space allocated @runQSGHS.\n");
    va_end(arg);
}
