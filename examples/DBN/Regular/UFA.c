...
int main(int argc, char **argv){
    ...
    switch (op){
        case 1:
            runUFA(F, Bernoulli_BernoulliDBN4Reconstruction, BBDBN_CD, Train, n_epochs, batch_size, n_gibbs_sampling, n_layers);
        break;
        case 2:
            runUFA(F, Bernoulli_BernoulliDBN4Reconstruction, BBDBN_PCD, Train, n_epochs, batch_size, n_gibbs_sampling, n_layers);
        break;
        case 3:
            runUFA(F, Bernoulli_BernoulliDBN4Reconstruction, BBDBN_FPCD, Train, n_epochs, batch_size, n_gibbs_sampling, n_layers);
        break;
    }   
    ...
    for(i = 0; i < n_layers; i++){
        gsl_vector_set(n_hidden_units, i, gsl_matrix_get(F->x, F->best, j));
        j+=4;
    }
    ...
    for(i = 0; i < d->n_layers; i++){
        d->m[i]->eta = gsl_matrix_get(F->x, F->best, j); j++;
        d->m[i]->lambda = gsl_matrix_get(F->x, F->best, j); j++;
        d->m[i]->alpha = gsl_matrix_get(F->x, F->best, j); j+=2;
