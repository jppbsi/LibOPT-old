...
int main(int argc, char **argv){
    ...
    switch (op){
        case 1:
            runAIWPSO(S, Bernoulli_BernoulliDBN4ReconstructionwithDropout, BBDBN_CD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling, n_layers);
        break;
        case 2:
            runAIWPSO(S, Bernoulli_BernoulliDBN4ReconstructionwithDropout, BBDBN_PCD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling, n_layers);
        break;
        case 3:
            runAIWPSO(S, Bernoulli_BernoulliDBN4ReconstructionwithDropout, BBDBN_FPCD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling, n_layers);
        break;
    }   
    ...
    for(i = 0; i < d->n_layers; i++){
        ...
        d->m[i]->alpha = gsl_vector_get(S->g, j); j++;
        gsl_vector_set(p, i, gsl_vector_get(S->g, j)); j++;
        gsl_vector_set(q, i, gsl_vector_get(S->g, j)); j+=2;
