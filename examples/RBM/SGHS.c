...
int main(int argc, char **argv){
    ...
    if(argc != 11){
        fprintf(stderr,"\nusage SGHS <training set> <test set> <output results file name> <cross-validation iteration number> \
    ...
    switch (op){
        case 1:
            runSGHS(H, Bernoulli_BernoulliRBM4ReconstructionwithDropout, BBRBM_CD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling);
        break;
        case 2:
            runSGHS(H, Bernoulli_BernoulliRBMbyPersistentContrastiveDivergencewithDropout, BBRBM_PCD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling);
        break;
        case 3:
            runSGHS(H, Bernoulli_BernoulliRBMbyFastPersistentContrastiveDivergencewithDropout, BBRBM_FPCD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling);
        break;
    }
    
    fprintf(stderr,"\nRunning RBM once more over the training set ... ");
    
    n_hidden_units = gsl_matrix_get(H->HM, H->best, 0);
    ...
    p = gsl_matrix_get(H->HM, H->best, 4);
    q = gsl_matrix_get(H->HM, H->best, 5);
