...
int main(int argc, char **argv){
    ...
    if(argc != 11){
        fprintf(stderr,"\nusage AIWPSO <training set> <test set> <output results file name> <cross-validation iteration number> \
                <swarm configuration file> <output best parameters file name> <n_epochs> <batch_size> \
    ...
    switch (op){
        case 1:
            runAIWPSO(S, Bernoulli_BernoulliRBM4ReconstructionwithDropout, BBRBM_CD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling);
        break;
        case 2:
            runAIWPSO(S, Bernoulli_BernoulliRBMbyPersistentContrastiveDivergencewithDropout, BBRBM_PCD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling);
        break;
        case 3:
            runAIWPSO(S, Bernoulli_BernoulliRBMbyFastPersistentContrastiveDivergencewithDropout, BBRBM_FPCD_DROPOUT, Train, n_epochs, batch_size, n_gibbs_sampling);
        break;
    }
    
    fprintf(stderr,"\nRunning RBM once more over the training set ... ");
    
    n_hidden_units = gsl_vector_get(S->g, 0);
    
    m = CreateRBM(Train->nfeats, n_hidden_units, 1);
    
    m->eta = gsl_vector_get(S->g, 1);
    m->lambda = gsl_vector_get(S->g, 2);
    m->alpha = gsl_vector_get(S->g, 3);
    p = gsl_vector_get(S->g, 4);
    q = gsl_vector_get(S->g, 5);
    m->eta_min = gsl_vector_get(S->LB, 1);
    m->eta_max = gsl_vector_get(S->UB, 1);  
