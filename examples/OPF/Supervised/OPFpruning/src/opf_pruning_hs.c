...
fprintf(stderr, 
"\nusage opf_ensemble_pruning [options] training_file evaluation_file(*) test_file\n\
Options:\n\
   -h [required] (harmony search): define path of parameters file to use in Harmony Search approach.\n\
   -a (optimization approach):\n\
      (0) - Harmony Search Binary\n\
      (1) - Harmony Search weighted (default)\n\
   -o (output): Output ensemble pruning classifier (default: ensemble_pruning)\n\n"
);
exit(1);
}
...
int main(int argc, char **argv){
	...
	int i,j, qtd_labels=0, ntraining = 0, optimization_option = 0, *poll_label = NULL, *psi = NULL, HS_type = 0;
	float time;
	float Acc;
	char fileName[256]; 
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'a':
				...
				if(optimization_option == 0 ) printf("\nEnsemble OPF using Binary Harmony Search");
				else if(optimization_option == 1 ){
	...
	if(!HS_type) InitializeHarmonyMemory_Binary(H);
	else InitializeHarmonyMemory(H);
	fprintf(stderr,"\nOK\n");

	psi = (int *)calloc((H->n),sizeof(int));
    ...
    for(i = 0; i <= ntraining; i++){
        ...
        if(training_p == 0.5){
            ...
            DestroySubgraph(&gTraining);
            //gTraining = CreateSubgraph(gAux->nnodes);
    ...
    for(i = 0; i < ntraining; i++){
        ...
        sprintf(fileName,"classifier_ensemble-%i.opf",i+1);
        opf_WriteModelFile(gTrain[i], fileName);
        fprintf(stdout, " OK"); fflush(stdout);

        fprintf(stdout, "\nWriting output file ..."); fflush(stdout);
        sprintf(fileName,"%s_opf-%i.out",argv[train_set],i);
        f = fopen(fileName,"w");
	...
	if(!HS_type) runHS_binary(H, ensemble_pruning, OPF_ENSEMBLE, gEval, gTrain, HS_type);
	else runHS(H, ensemble_pruning, OPF_ENSEMBLE, gEval, gTrain, HS_type);
	...
	if(!fParameters) fParameters = fopen("best_ensemble.txt", "a");
	    ...
	    for(i = 0; i < H->n; i++){
			...
			//PAREI AQUI.... ESTÁ RETORNANDO DOUBLE PARA W. AJUSTAR OUTPUT PARA BINÁRIO E PESO.
			
	        psi[i] = (int)gsl_matrix_get(H->HM, H->best, i); //Set classifiers to testing phase
	        if(psi[i]) printf("clf[%i], ",i);
	}
	fprintf(fParameters, "\n");
	fclose(fParameters);
