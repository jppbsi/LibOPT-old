...
int main(int argc, char **argv){
	...
	for(i=1;i<argc;i++)
	{
		...
		{
			case 'p':
				fprintf(stdout, "\nLoading Cuchoo Search parameters file [%s] ...", argv[i]); fflush(stdout);
				P = ReadNestPopulationFromFile(argv[i]);
	...
	ShowNestPopulationInformation(P);
	...
	runCS(P, ensemble_pruning, OPF_ENSEMBLE, gEval, gTrain, binary_optimization);
    ...
    for(i = 0; i < P->n; i++){
        fprintf(fParameters, "%lf:(%d) ", gsl_matrix_get(P->x, P->best_fitness, i),(int)psi[i]);
	}
	fprintf(fParameters, "\n");
	fclose(fParameters);
