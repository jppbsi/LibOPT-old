...
void info(){
    fflush(stdout);
	fprintf(stdout, "\n\nProgram that executes EPNN-OPF with PSO optimization\n");
...
fprintf(stderr, 
"\nusage epnn_pso [options] training_file evaluation_file test_file\n\
Options:\n\
	-c [required] (model file for OPF-CLUSTER): define path of parameters file to use in swarm optimization for OPF-CLUSTER.\n\
	-e [required] (model file for EPNN): define path of parameters file to use in swarm optimization for EPNN.\n\
...
int main(int argc, char **argv){
	...
	float Acc, sigma = 0.0, radius = 0.0;
	char fileName[256]; 
	FILE *f = NULL, *fParameters = NULL;
	timer tic, toc;
	Swarm *S1 = NULL, *S2 = NULL;
	...
	for(i=1;i<argc;i++)
	{
		...
		{
			case 'c':
				fprintf(stdout, "\nLoading Swarm parameters file [%s] for OPF-CLUSTER ...", argv[i]); fflush(stdout);
				S1 = ReadSwarmFromFile(argv[i]);
				fprintf(stdout, " OK\n"); fflush(stdout);				
				break;
		
			case 'e':
				fprintf(stdout, "\nLoading Swarm parameters file [%s] for EPNN ...", argv[i]); fflush(stdout);
				S2 = ReadSwarmFromFile(argv[i]);
				fprintf(stdout, " OK\n"); fflush(stdout);				
				break;
	...
	if(S1){
		...
		fprintf(stdout,"\nInitializing swarm for OPF-CLUSTER... ");
		InitializeSwarm(S1);
		fprintf(stdout,"\nOK\n");
	}
	...
	if(S1){
		...
		runPSO(S1, OPFclusterOptimization, OPFKNN, gTrain, gEval); gettimeofday(&toc,NULL);
		fprintf(stdout, " OK\n"); fflush(stdout);
		...
		kmax = (int)gsl_matrix_get(S1->x, S1->best_fitness, 0);
	...
	if(S2){
 	...
 	if(S2){	
	    ...
	    gettimeofday(&tic,NULL);
		runPSO(S2, EPNNoptimization, EPNN_OPF, gTrain, gEval, lNode, nsample4class, nGaussians); gettimeofday(&toc,NULL);
		fprintf(stdout, " OK\n"); fflush(stdout);
		
		ShowSwarm(S2);
		
		sigma = gsl_matrix_get(S2->x, S2->best_fitness, 0);
		radius = gsl_matrix_get(S2->x, S2->best_fitness, 1);
