...
Options:\n\
	-c [required] (model file for OPF-CLUSTER): define path of parameters file to use in Cuckoo Search approach for OPF-CLUSTER.\n\
	-e [required] (model file for EPNN): define path of parameters file to use in Cuckoo Search approach for EPNN.\n\
...
int main(int argc, char **argv){
	...
	timer tic, toc;
	NestPopulation *P1 = NULL, *P2 = NULL;
	...
	for(i=1;i<argc;i++)
	{
		...
		{
			case 'c':
				fprintf(stdout, "\nLoading Cuchoo Search parameters file [%s] for OPF-CLUSTER ...", argv[i]); fflush(stdout);
				P1 = ReadNestPopulationFromFile(argv[i]);
				fprintf(stdout, " OK\n"); fflush(stdout);				
				break;
		
			case 'e':
				fprintf(stdout, "\nLoading Cuckoo Search parameters file [%s] for EPNN ...", argv[i]); fflush(stdout);
				P2 = ReadNestPopulationFromFile(argv[i]);
				fprintf(stdout, " OK\n"); fflush(stdout);				
				break;
	...
	// LOADING NEST POPULATION
	if(P1){
		ShowNestPopulationInformation(P1);
	
		fprintf(stdout,"\nInitializing nest population for OPF-CLUSTER... ");
		InitializeNestPopulation(P1);
		fprintf(stdout,"\nOK\n");
	}
	...
	if(P1){
		...
		runCS(P1, OPFclusterOptimization, OPFKNN, gTrain, gEval); gettimeofday(&toc,NULL);
		fprintf(stdout, " OK"); fflush(stdout);
	...
	// LOADING NEST POPULATION
	if(P2){
		ShowNestPopulationInformation(P2);

		fprintf(stdout,"\nInitializing nest population for EPNN... ");
		InitializeNestPopulation(P2);
		fprintf(stdout,"\nOK\n");
	}
 	...
 	if(P2){	
	    ...
	    gettimeofday(&tic,NULL);
		runCS(P2, EPNNoptimization, EPNN_OPF, gTrain, gEval, lNode, nsample4class, nGaussians); gettimeofday(&toc,NULL);
		fprintf(stdout, " OK"); fflush(stdout);
    ...
    fflush(stdout);
    fprintf(stdout,"\nDeallocating memory ... ");
    DestroyNestPopulation(&P1);
	DestroyNestPopulation(&P2);
