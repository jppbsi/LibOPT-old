...
Options:\n\
	-c [required] (model file for OPF-CLUSTER): define path of parameters file to use in Harmony Search approach for OPF-CLUSTER.\n\
	-e [required] (model file for EPNN): define path of parameters file to use in Harmony Search approach for EPNN.\n\
...
int main(int argc, char **argv){
	...
	if(H1){
		...
		runHS(H1, OPFclusterOptimization, OPFKNN, gTrain, gEval); gettimeofday(&toc,NULL);
		fprintf(stdout, " OK"); fflush(stdout);
	...
	if(H2){
 	...
 	if(H2){	
	    ...
	    gettimeofday(&tic,NULL);
		runHS(H2, EPNNoptimization, EPNN_OPF, gTrain, gEval, lNode, nsample4class, nGaussians); gettimeofday(&toc,NULL);
		fprintf(stdout, " OK"); fflush(stdout);
